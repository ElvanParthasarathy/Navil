...
const CategoryListView = () => {
    ...
    const ITEMS_PER_PAGE = (category === 'poems' || category === 'quotes' || category === 'stories') ? 6 : 5;
    ...
    const groupedItems = React.useMemo(() => {
        ...
        resultItems.sort((a, b) => new Date(b.publishDate || b.date || b.post?.date || 0).getTime() - new Date(a.publishDate || a.date || a.post?.date || 0).getTime());
    ...
    return (
        <>
            {isDesktop && activePortalTarget && createPortal(renderControls(true), activePortalTarget)}
            ...
            <Helmet>
                ...
                <link rel="canonical" href={`https://elvannavil.vercel.app/writings/${category}`} />
            </Helmet>

            {/* Filters & Search Sync for Mobile / Non-Desktop */}
                    ...
                    currentItems.map((item, index) => {
                        if (item.type === 'series') {
                            ...
                            if (firstHasVariants) {
                                ...
                                primaryExcerpt = (textHtml ? (new DOMParser().parseFromString(textHtml, 'text/html').body.textContent || '') : '').substring(0, 120);
                            } else {

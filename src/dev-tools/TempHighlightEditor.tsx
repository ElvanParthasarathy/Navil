...
const TempHighlightEditor = ({ initialHighlights }) => {
    ...
    const updateHighlightTitle = (index, newTitle) => {
        const newArr = [...highlights];
        newArr[index] = { ...newArr[index], title: newTitle };
        setHighlights(newArr);
    };

    const sortStories = (hIndex, direction) => {
        const newArr = [...highlights];
    ...
    const selectAllInHighlight = (hIndex) => {
    ...
    const moveStory = (fromHighlightIndex, storyIndex, toHighlightId) => {
    ...
    const setAsCover = (highlightIndex, storyUrl, e) => {
        e.stopPropagation();
        const newArr = [...highlights];
    ...
    // Drag & Drop
    const onDragStart = (e, hIndex, sIndex) => {
    ...
    const onDrop = (e, dropHIndex, dropSIndex) => {
        e.preventDefault();
    ...
    const onDropContainer = (e, dropHIndex) => {
        e.preventDefault();
        if (!draggedItem) return;
        ...
        if (e.target.className.includes('stories-container')) {
            const newHighlights = JSON.parse(JSON.stringify(highlights));
            const [moved] = newHighlights[dragHIndex].stories.splice(dragSIndex, 1);
